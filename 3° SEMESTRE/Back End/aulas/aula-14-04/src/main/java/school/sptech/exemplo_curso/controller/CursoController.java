...
public class CursoController {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<CursoListagemDto> atualizaatualizar(@PathVariable Integer id, @Valid @RequestBody CursoAtualizacaoDto dto) {
    ...
    @DeleteMapping("/deletar/precos")
    public ResponseEntity<Void> remover(@RequestParam Double preco) {
        cursoService.removerPorPreco(preco);
        return ResponseEntity.status(204).build();
    }
}
